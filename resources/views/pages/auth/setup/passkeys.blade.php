...
<section class="relative px-4 py-5 mx-auto w-full max-w-(--breakpoint-lg)">
            ...
            <x-auth::setup.heading title="Passkeys" description="Enable passwordless sign-in with passkeys on your authentication screens." />
                ...
                @else
                    <div class="p-5 mb-6 text-sm border rounded-lg border-zinc-200 bg-zinc-50">
                        ...
                        <ul class="mt-2 space-y-1 list-disc list-inside text-zinc-600">
                            <li>Publish the passkeys config: <code class="text-xs">php artisan vendor:publish --tag=auth:passkeys-config</code></li>
                            <li>Run the passkeys migration: <code class="text-xs">php artisan vendor:publish --tag=passkeys-migrations --provider="Laravel\Passkeys\PasskeysServiceProvider"</code></li>
                    ...
                    <div class="flex relative justify-between items-center max-w-sm border-b border-b-zinc-200">
                        ...
                        <div class="flex relative items-center">
                            <x-auth::setup.checkbox wire:change="update('enable_passkeys', $event.target.checked)" :checked="($settings->enable_passkeys ? true : false)" />
                        </div>
                    </div>
                @endif
            </div>
